...
<div class="relative min-h-screen">
    ...
    <div class="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
        <div>
            <h1 class="text-3xl font-black text-slate-950 tracking-tight">Sovereign Console</h1>
            <p class="text-slate-500 font-medium">Identity orchestration and system-wide security audit</p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 relative z-10">
        ...
        <div class="lg:col-span-8 bg-white/60 backdrop-blur-xl rounded-[2.5rem] border border-white/40 shadow-xl shadow-slate-200/40 overflow-hidden">
            ...
            <div class="p-8 space-y-4">
                ...
                <div class="flex items-center gap-5 p-5 rounded-3xl bg-white/40 border border-slate-100 hover:border-emerald-200 hover:bg-emerald-50/30 transition-all group">
                    <div class="w-14 h-14 rounded-2xl bg-gradient-to-br from-emerald-50 to-sky-50 flex items-center justify-center text-xl font-black text-slate-500 group-hover:from-emerald-600 group-hover:to-sky-500 group-hover:text-white transition-all shadow-sm">
                        {{ substr($user->name, 0, 1) }}
                    </div>
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 mb-1">
                            <p class="font-black text-slate-950 tracking-tight text-lg">{{ $user->name }}</p>
                            <span class="text-[9px] px-2.5 py-1 rounded-lg bg-slate-900 text-white uppercase font-black tracking-widest shadow-lg shadow-slate-900/10">
                                {{ $user->roles->first()?->name ?? 'No Role' }}
                            </span>
                        </div>
                        <p class="text-xs text-slate-400 font-bold tracking-tight">
                            {{ $user->email }} · <span class="text-slate-300 italic font-medium">{{ $user->username }}</span>
                        </p>
                    </div>
                    <div class="flex items-center gap-4">
                        <form action="{{ route('admin.users.toggle-status', $user) }}" method="POST">
                            @csrf
                            <button type="submit" 
                                    class="relative inline-flex h-6 w-12 shrink-0 cursor-pointer items-center rounded-full transition-all duration-300 {{ $user->is_active ? 'bg-emerald-500 shadow-lg shadow-emerald-500/20' : 'bg-slate-200' }}">
                        ...
                        <button class="w-10 h-10 flex items-center justify-center rounded-2xl bg-white border border-slate-200 text-slate-400 hover:text-emerald-600 hover:border-emerald-100 hover:shadow-lg transition-all active:scale-90 shadow-sm">
        ...
        <!-- <div class="lg:col-span-4 bg-slate-950 rounded-[2.5rem] shadow-2xl shadow-slate-900/20 overflow-hidden text-white flex flex-col">
            ...
            <div class="p-8 flex-1 space-y-8 relative overflow-y-auto max-h-[600px] custom-scrollbar">
                ...
                <div class="relative pl-12 group">
                    <div class="absolute left-[7px] top-1.5 w-3 h-3 rounded-full bg-slate-950 border-2 border-emerald-500 shadow-[0_0_12px_rgba(16,185,129,0.4)] z-10 transition-transform group-hover:scale-125"></div>
                    <div class="space-y-1">
                        ...
                        <p class="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">{{ $log->module }} · {{ $log->timestamp->diffForHumans() }}</p>
