...
<div class="space-y-8">
    ...
    <div class="flex items-center gap-4">
        <x-button variant="ghost" size="md" href="{{ route('purchases.index') }}" class="!p-2">
        ...
        <div>
            ...
            <p class="text-sm text-slate-500 font-medium mt-1 uppercase tracking-widest italic">Stock Inward Record</p>
        </div>
    </div>

    <div class="max-w-5xl">
        <form action="{{ route('purchases.store') }}" method="POST" class="space-y-8">
            @csrf
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <x-card class="relative overflow-hidden">
                    ...
                    <div class="space-y-6">
                        <x-input label="Vendor Firm Name *" name="vendor_name" value="{{ $vendor_name ?? '' }}" required placeholder="e.g. Premium Feeds Ltd" />
                        
                        <div class="grid grid-cols-2 gap-6">
                            <x-input label="Inward Date *" type="date" name="date" value="{{ date('Y-m-d') }}" required />
                            <div class="space-y-2">
                                <label class="text-[11px] font-bold text-slate-500 uppercase tracking-widest px-1">Payment Channel</label>
                                <select name="payment_mode" class="w-full bg-slate-50 border-slate-200 rounded-2xl py-3 px-5 text-sm font-bold text-slate-700 outline-none focus:ring-4 focus:ring-primary-500/10 transition-all border">
                ...
                {{-- Item Details --}}
                <x-card class="relative overflow-hidden">
                    <div class="absolute top-0 right-0 p-6 opacity-5 text-emerald-500">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            <x-input label="Quantity *" type="number" name="quantity" id="qty" step="0.01" required placeholder="0.00" />
                            <x-input label="Unit Measure" name="unit" value="kg" placeholder="kg, bags, etc" />
                        </div>

                        <x-input label="Rate per Unit (₹) *" type="number" name="rate" id="rate" step="0.01" required placeholder="0.00" />
                    </div>
                </x-card>
            </div>
            ...
            <x-card class="bg-slate-900 border-none shadow-2xl relative overflow-hidden group">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-12 relative items-center">
                    <div class="space-y-4">
                        ...
                        <input type="number" name="gst_percentage" id="gst_p" value="18" step="0.1" 
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex items-baseline gap-2">
                            ...
                            <input type="number" name="gst_amount" id="gst_a" step="0.01" readonly 
                    ...
                    <div class="bg-white/5 rounded-[2.5rem] p-8 border border-white/10 backdrop-blur-md">
                        ...
                        <div class="flex items-baseline gap-2">
                            ...
                            <input type="number" name="total_amount" id="total" step="0.01" required readonly 
            ...
            <div class="flex flex-col md:flex-row justify-end items-center gap-6">
                <button type="reset" class="text-xs font-black uppercase tracking-widest text-slate-400 hover:text-rose-500 transition-colors">Wipe Entry</button>
                <x-button variant="primary" size="lg" type="submit" class="w-full md:w-auto px-16 py-5 rounded-[2rem] shadow-primary-500/20">
                    <x-slot name="icon"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" /></x-slot>
                    Commit Purchase
                </x-button>
            </div>
        </form>
