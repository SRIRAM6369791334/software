    ...
    <div class="grid grid-cols-2 gap-12 mb-10">
        ...
        <div class="text-right">
            ...
            <div class="space-y-2">
                ...
                <p class="text-sm text-slate-600">Period: <span class="font-semibold text-slate-950 italic">{{ $bill->period_start?->format('d M') ?? 'N/A' }} - {{ $bill->period_end?->format('d M Y') ?? 'N/A' }}</span></p>
    ...
    <div class="mb-10 overflow-hidden rounded-xl border border-slate-200">
        <table class="w-full text-left">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr>
                    <td class="px-6 py-5">
                        <p class="text-sm font-bold text-slate-950">{{ $item->item_name }}</p>
                        <p class="text-xs text-slate-400 mt-0.5">Weekly supply item</p>
                    </td>
                    <td class="px-6 py-5 text-right font-mono text-sm text-slate-600">{{ number_format($item->quantity_kg, 2) }} kg</td>
                    <td class="px-6 py-5 text-right font-mono text-sm text-slate-600">Rs {{ number_format($item->rate_per_kg, 2) }}</td>
                    <td class="px-6 py-5 text-right font-mono font-bold text-slate-950">Rs {{ number_format($item->quantity_kg * $item->rate_per_kg, 2) }}</td>
                </tr>
                @empty
                <tr>
                    <td class="px-6 py-5">
                        <p class="text-sm font-bold text-slate-950">{{ $bill->items_description ?: 'Livestock/Poultry Products' }}</p>
                        <p class="text-xs text-slate-400 mt-0.5">Weekly supply for cycle {{ $bill->period_start?->format('M d') ?? 'N/A' }}</p>
                    </td>
    ...
    <div class="flex justify-end mb-12">
        <div class="w-64 bg-gradient-to-r from-emerald-50/80 to-sky-50/80 rounded-2xl p-6 border border-slate-200">
            ...
            <div class="flex justify-between items-center mb-4 pb-4 border-b border-slate-200">
                <span class="text-xs text-slate-500 font-medium">Tax ({{ $bill->gst_percentage ?? 0 }}%)</span>
                <span class="text-sm font-mono text-slate-950 font-semibold">Rs {{ number_format($bill->gst_amount ?? 0, 2) }}</span>
            </div>
