...
<div class="max-w-3xl mx-auto bg-gradient-to-br from-white via-emerald-50/30 to-sky-50/30 p-8 border border-zinc-200 shadow-lg rounded-xl my-4" id="invoice-print">
    ...
    <div class="grid grid-cols-2 gap-12 mb-10">
        ...
        <div class="text-right">
            ...
            <div class="space-y-2">
                <p class="text-sm text-zinc-600">Date: <span class="font-semibold text-zinc-950">{{ date('d M Y') }}</span></p>
                <p class="text-sm text-zinc-600">Period: <span class="font-semibold text-zinc-950 italic">{{ $bill->period_start?->format('d M') ?? 'N/A' }} - {{ $bill->period_end?->format('d M Y') ?? 'N/A' }}</span></p>
                <p class="text-sm text-zinc-600">Status: <span class="px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 text-[10px] font-bold uppercase">{{ $bill->status }}</span></p>
            </div>
        </div>
    </div>
    ...
    <div class="mb-10 overflow-hidden rounded-xl border border-zinc-200">
        <table class="w-full text-left">
            ...
            <tbody class="divide-y divide-zinc-100">
                ...
                <tr>
                    <td class="px-6 py-5">
                        <p class="text-sm font-bold text-zinc-950">{{ $item->item_name }}</p>
                        <p class="text-xs text-zinc-400 mt-0.5">Weekly supply item</p>
                    </td>
                    <td class="px-6 py-5 text-right font-mono text-sm text-zinc-600">{{ number_format($item->quantity_kg, 2) }} kg</td>
                    <td class="px-6 py-5 text-right font-mono text-sm text-zinc-600">Rs {{ number_format($item->rate_per_kg, 2) }}</td>
                    <td class="px-6 py-5 text-right font-mono font-bold text-zinc-950">Rs {{ number_format($item->quantity_kg * $item->rate_per_kg, 2) }}</td>
                </tr>
                @empty
                <tr>
                    <td class="px-6 py-5">
                        <p class="text-sm font-bold text-zinc-950">{{ $bill->items_description ?: 'Livestock/Poultry Products' }}</p>
                        <p class="text-xs text-zinc-400 mt-0.5">Weekly supply for cycle {{ $bill->period_start?->format('M d') ?? 'N/A' }}</p>
                    </td>
                    <td class="px-6 py-5 text-right font-mono text-sm text-zinc-600">{{ number_format($bill->quantity_kg, 2) }} kg</td>
                    <td class="px-6 py-5 text-right font-mono text-sm text-zinc-600">Rs {{ number_format($bill->amount / ($bill->quantity_kg ?: 1), 2) }}</td>
                    <td class="px-6 py-5 text-right font-mono font-bold text-zinc-950">Rs {{ number_format($bill->amount, 2) }}</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="flex justify-end mb-12">
        <div class="w-64 bg-gradient-to-r from-emerald-50/80 to-sky-50/80 rounded-2xl p-6 border border-zinc-200">
            <div class="flex justify-between items-center mb-3">
                ...
                <span class="text-sm font-mono text-zinc-950 font-semibold">Rs {{ number_format($bill->amount, 2) }}</span>
            </div>
            <div class="flex justify-between items-center mb-4 pb-4 border-b border-zinc-200">
                <span class="text-xs text-zinc-500 font-medium">Tax ({{ $bill->gst_percentage ?? 0 }}%)</span>
                <span class="text-sm font-mono text-zinc-950 font-semibold">Rs {{ number_format($bill->gst_amount ?? 0, 2) }}</span>
            </div>
            <div class="flex justify-between items-center">
                <span class="text-sm font-bold text-zinc-950 uppercase tracking-wider">Total Due</span>
                <span class="text-xl font-black text-emerald-600 font-mono">Rs {{ number_format($bill->net_amount ?? $bill->amount, 2) }}</span>
            </div>
        </div>
    </div>
