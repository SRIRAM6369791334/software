...
<div class="flex items-center justify-between mb-6">
    <div>
        <h1 class="text-2xl font-bold text-slate-950">Daily Purchase Report</h1>
        <p class="text-sm text-slate-500 mt-0.5">Summary for {{ \Carbon\Carbon::parse($date)->format('d F, Y') }}</p>
    </div>
    <div class="flex gap-2">
        ...
        <a href="{{ route('reports.purchases.export-pdf', ['date' => $date]) }}" class="cm-export-btn">
...
<div class="bg-gradient-to-br from-white via-emerald-50/30 to-sky-50/30 p-5 rounded-xl border border-slate-200 shadow-sm mb-6">
    <form action="{{ route('reports.purchases.daily') }}" method="GET" class="flex items-end gap-4">
        <div class="flex-1 max-w-xs">
            <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Select Date</label>
...
<div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
    <div class="bg-gradient-to-br from-white via-emerald-50/30 to-sky-50/30 p-5 rounded-xl border border-slate-200 shadow-sm">
        ...
        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2">Total Purchase Amount</p>
    ...
    <div class="bg-gradient-to-br from-white via-emerald-50/30 to-sky-50/30 p-5 rounded-xl border border-slate-200 shadow-sm">
        <span class="text-xl"></span>
        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2">Total Items</p>
        <p class="text-xl font-bold text-blue-600 mt-0.5">{{ $purchases->count() }}</p>
    </div>
</div>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-slate-200 bg-slate-50">
                    ...
                    <th class="px-6 py-4 text-center text-xs font-semibold text-slate-400 uppercase tracking-wider">Qty</th>
                    <th class="px-6 py-4 text-right text-xs font-semibold text-slate-400 uppercase tracking-wider">Rate</th>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-gradient-to-r from-emerald-50/80 to-sky-50/80 transition-colors">
                    ...
                    <td class="px-6 py-4 text-center font-mono text-slate-600">{{ $purchase->quantity }}</td>
                    <td class="px-6 py-4 text-right font-mono text-slate-500">Rs {{ number_format($purchase->rate, 0, '.', ',') }}</td>
