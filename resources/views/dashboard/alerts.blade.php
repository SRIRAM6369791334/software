...
@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-bold text-slate-950">System Alerts & Notifications</h1>
    <p class="text-sm text-slate-500 mt-0.5">Track upcoming payments and critical system business dates</p>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-4">
        <div class="bg-gradient-to-br from-white via-emerald-50/40 to-sky-50/40 rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="px-5 py-4 border-b border-slate-200 bg-amber-50/30 flex justify-between items-center">
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="text-left border-b border-slate-200 bg-gradient-to-r from-emerald-50/80 to-sky-50/80">
                            <th class="px-5 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Item/Asset</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($upcomingEmis as $emi)
                            <tr class="hover:bg-gradient-to-r from-emerald-50/80 to-sky-50/80 transition-colors">
                                <td class="px-5 py-4">
                                    <p class="font-bold text-slate-950">{{ $emi->item }}</p>
                                    <p class="text-xs text-slate-500">{{ $emi->note ?? 'No description' }}</p>
                                </td>
                                <td class="px-5 py-4 text-slate-600">
                                    <span class="{{ $emi->due_date->isToday() ? 'text-red-600 font-bold' : '' }}">
                                        {{ $emi->due_date->format('d M Y') }}
                                    </span>
                                    <p class="text-[10px] uppercase font-bold text-slate-400 mt-0.5">{{ $emi->due_date->diffForHumans() }}</p>
                                </td>
                                <td class="px-5 py-4 text-right font-mono font-bold text-slate-950">
                                    Rs {{ number_format($emi->amount, 0) }}
                                </td>
                        ...
                        @empty
                            <tr><td colspan="4" class="px-5 py-12 text-center text-slate-400 italic">No upcoming EMIs found for the next 30 days.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
    ...
    <div class="space-y-6">
        <div class="bg-gradient-to-br from-white via-emerald-50/40 to-sky-50/40 rounded-xl border border-slate-200 shadow-sm p-5">
            <h3 class="text-sm font-bold text-slate-950 uppercase tracking-widest border-b border-slate-200 pb-3 mb-4 flex items-center gap-2">
                <span></span> System Status
            </h3>
            <div class="space-y-4">
                <div class="flex items-center gap-3">
                    <div class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
                    <p class="text-xs text-slate-600">Database connected & synchronized</p>
                </div>
                <div class="flex items-center gap-3">
                    <div class="w-2 h-2 rounded-full bg-emerald-500"></div>
                    <p class="text-xs text-slate-600">Daily backups completed (02:00 AM)</p>
                </div>
            </div>
        </div>

        <div class="bg-primary rounded-xl shadow-lg p-5 text-white">
