@extends('layouts.app')
@section('title', 'Edit Dealer Payment')

@section('content')
<div class="animate-fade-in max-w-4xl mx-auto">
    <x-page-header title="Edit Payment" subtitle="Update payment details and synchronize balances">
        <x-slot:actions>
    ...
    <div class="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border border-zinc-200/60 dark:border-zinc-800/60 shadow-[0_8px_32px_rgba(0,0,0,0.04)] rounded-3xl overflow-hidden p-6 sm:p-10">
        ...
        <form action="{{ route('payments.dealers.update', $payment->id) }}" method="POST" class="space-y-8" 
              x-data="{ 
                    ...
                    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                        <div>
                            ...
                            <div class="text-xl font-black text-zinc-900 dark:text-white">
                                {{ $dealer->firm_name ?? 'Dealer' }}
                            </div>
                            <span class="text-xs text-zinc-400">Contact: {{ $dealer->contact_person ?? '-' }} ({{ $dealer->phone ?? '-' }})</span>
                        </div>
                        <div class="text-left sm:text-right">
                            ...
                            <div class="text-xl font-cabinet font-black text-rose-600 dark:text-rose-400">
                                <x-currency :amount="$dealer->displayed_outstanding ?? 0" />
                            </div>
                        </div>
                    </div>
            ...
            <div class="flex items-center justify-end gap-4 pt-6 border-t border-zinc-100 dark:border-zinc-800">
                ...
                <x-button type="submit" variant="primary" icon="save" size="lg" class="shadow-xl shadow-blue-500/20 px-8 !bg-blue-600 hover:!bg-blue-700">Save & Update</x-button>
            </div>
