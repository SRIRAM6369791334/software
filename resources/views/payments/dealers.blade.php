...
<div class="space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Dealer Payments</h1>
            <p class="text-sm text-slate-500 font-medium mt-1">Audit trail and record management for dealer payables</p>
        </div>
        <div class="flex items-center gap-3">
            ...
            <x-button variant="secondary" size="md" href="{{ route('payments.dealers.export') }}">
    ...
    <div class="max-w-md">
        <form method="GET" class="relative group">
            ...
            <input type="text" name="search" value="{{ $search }}" placeholder="Search by dealer firm name..." 
    ...
    <x-card padding="false">
        <div class="px-8 py-6 border-b border-slate-100 bg-slate-50/50">
            <h2 class="text-sm font-black text-slate-900 uppercase tracking-wider">Outflow Registry</h2>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead>
                    <tr class="text-left border-b border-slate-100 bg-slate-50/20">
                        <th class="px-8 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">Dealer Entity</th>
                        <th class="px-8 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">Post Date</th>
                        <th class="px-8 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] text-right">Outflow Amount</th>
                        <th class="px-8 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] text-center">Channel</th>
                        <th class="px-8 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] text-right">Owed Balance After</th>
                        <th class="px-8 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] text-center">Audit</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-50">
                    @forelse($payments as $p)
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            ...
                            <td class="px-8 py-6 text-right">
                                <p class="text-base font-black text-rose-600">₹{{ number_format($p->amount, 0) }}</p>
                            </td>
                            ...
                            <td class="px-8 py-6 text-right">
                                <p class="text-xs font-bold text-slate-400 font-mono italic">₹{{ number_format($p->pending_balance_after, 0) }}</p>
                            </td>
                            <td class="px-8 py-6 text-center">
                                <a href="{{ route('payments.dealers.ledger', $p->dealer_id) }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider text-primary-600 bg-primary-50 hover:bg-primary-500 hover:text-white transition-all ring-1 ring-primary-100">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-8 py-12 text-center text-slate-400 font-medium italic">No recorded dealer payments found</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
<div id="add-payment-modal" class="hidden fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xl animate-in fade-in duration-300">
    <div class="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg border border-white/20 overflow-hidden transform animate-in zoom-in-95 duration-300">
        <div class="flex items-center justify-between px-10 py-8 border-b border-slate-100 bg-slate-50/50">
            <div>
                <h2 class="text-2xl font-black text-slate-900 tracking-tight">Record Outflow</h2>
                <p class="text-xs text-slate-500 font-bold uppercase tracking-widest mt-1">Dealer Payment Entry</p>
            </div>
        ...
        <form action="{{ route('payments.dealers.store') }}" method="POST" class="p-10 space-y-6">
            @csrf
            <div class="space-y-2">
                <label class="text-[11px] font-bold text-slate-500 uppercase tracking-widest px-1">Target Dealer *</label>
                <select name="dealer_id" required class="w-full bg-slate-50 border-slate-200 rounded-2xl py-3 px-5 text-sm font-bold text-slate-700 outline-none focus:ring-4 focus:ring-primary-500/10 transition-all">
                    <option value="">Select dealer entity…</option>
                    @foreach($dealers as $d)
                        <option value="{{ $d->id }}">{{ $d->firm_name }} (Owed: ₹{{ number_format($d->pending_amount, 0) }})</option>
                    @endforeach
                </select>
            </div>

            <div class="grid grid-cols-2 gap-6">
                <x-input label="Payment Amount (₹) *" type="number" name="amount" required step="0.01" min="0.01" placeholder="0.00" />
                <x-input label="Transaction Date *" type="date" name="date" required value="{{ date('Y-m-d') }}" />
            </div>

            <div class="space-y-2">
                <label class="text-[11px] font-bold text-slate-500 uppercase tracking-widest px-1">Payment Channel *</label>
            ...
            <x-input label="Reference / Notes" name="notes" placeholder="Transaction ID, Cheque #, etc" />

            <div class="pt-4 flex gap-4">
                <x-button variant="ghost" class="flex-1" type="button" onclick="toggleModal('add-payment-modal')">Discard</x-button>
                <x-button variant="primary" class="flex-1" type="submit">Finalize Outflow</x-button>
            </div>
        </form>
    </div>
</div>
