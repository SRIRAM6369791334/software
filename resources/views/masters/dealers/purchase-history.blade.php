...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-gray-900">Dealer Purchase History</h1>
    <p class="text-sm text-gray-500 mt-0.5">{{ $dealer->firm_name }} | Inventory inward tracking</p>
</div>

<div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
    {{-- Tabs --}}
    <div class="flex border-b border-gray-100 bg-gray-50/30">
        <a href="{{ route('masters.dealers.show', $dealer) }}" class="px-6 py-4 text-sm font-semibold text-gray-500 hover:text-gray-900">Overview</a>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="text-left border-b border-gray-100 bg-gray-50/50">
                    ...
                    <th class="px-5 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">Item Purchased</th>
            ...
            <tbody class="divide-y divide-gray-50">
                @forelse($purchases as $purchase)
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-5 py-4">
                            ...
                            <p class="text-[10px] text-gray-400 font-mono uppercase tracking-tighter">ID: #PUR-{{ $purchase->id }}</p>
                        </td>
                        <td class="px-5 py-4 text-right font-mono text-gray-600">{{ number_format($purchase->quantity, 2) }} {{ $purchase->unit }}</td>
                        <td class="px-5 py-4 text-right text-gray-600">₹{{ number_format($purchase->rate, 2) }}</td>
                ...
                @empty
                    <tr><td colspan="6" class="px-5 py-12 text-center text-gray-400 italic">No purchase records found for this dealer.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
