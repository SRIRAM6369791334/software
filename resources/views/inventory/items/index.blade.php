...
<div class="flex flex-col md:flex-row md:items-center justify-between mb-8">
    <div>
        <h1 class="text-2xl font-bold text-slate-950">Item Master</h1>
        <p class="text-sm text-slate-500 mt-0.5">Manage poultry resources and inventory definitions</p>
    </div>
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
    <div class="bg-gradient-to-br from-white via-emerald-50/30 to-sky-50/30 p-5 rounded-2xl border border-slate-200 shadow-sm flex items-start gap-4">
        ...
        <div>
            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Total Items</p>
            <h3 class="text-2xl font-black text-slate-950">{{ $items->total() }}</h3>
        </div>
    </div>
    <div class="bg-gradient-to-br from-white via-emerald-50/30 to-sky-50/30 p-5 rounded-2xl border border-slate-200 shadow-sm flex items-start gap-4">
        <div class="w-12 h-12 rounded-xl bg-amber-50 text-amber-600 flex items-center justify-center text-xl shadow-sm"></div>
        <div>
            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Chick Breeds</p>
    ...
    <div class="bg-gradient-to-br from-white via-emerald-50/30 to-sky-50/30 p-5 rounded-2xl border border-slate-200 shadow-sm flex items-start gap-4">
        ...
        <div>
            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Medications</p>
    ...
    <div class="bg-gradient-to-br from-white via-emerald-50/30 to-sky-50/30 p-5 rounded-2xl border border-slate-200 shadow-sm flex items-start gap-4">
        ...
        <div>
            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Equipments</p>
...
<div class="bg-gradient-to-br from-white via-emerald-50/30 to-sky-50/30 p-4 rounded-xl border border-slate-200 shadow-sm mb-6">
    <form action="{{ route('inventory.items.index') }}" method="GET" class="flex flex-wrap items-center gap-4">
        ...
        <div class="flex items-center gap-2">
            <label class="text-[10px] font-bold text-slate-400 uppercase">Filter Type:</label>
        ...
        <table class="w-full text-sm text-left">
            <thead>
                <tr class="border-b border-slate-100 bg-gradient-to-r from-emerald-50/70 to-sky-50/70">
                    <th class="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Item Description</th>
                    <th class="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Category & Type</th>
                    <th class="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Unit Logics</th>
                    <th class="px-6 py-4 text-right text-[10px] font-bold text-slate-400 uppercase tracking-widest">Current Stock</th>
                    ...
                    <tr class="hover:bg-slate-50/20 transition-colors group">
                        <td class="px-6 py-4">
                            <div class="flex flex-col">
                                ...
                                <span class="text-[10px] text-slate-400 font-bold uppercase tracking-tight">{{ $item->brand ?: 'Master Record' }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex flex-col gap-1">
                                <span class="px-2 py-0.5 w-max rounded text-[10px] font-black uppercase tracking-tighter
                                    ...
                                    {{ !in_array($item->type, ['Feed','Chick','Medicine']) ? 'bg-sky-50 text-slate-600' : '' }}">
                                    {{ $item->type }}
                                </span>
                                <span class="text-[10px] text-slate-400 font-medium px-0.5">{{ $item->category ?: 'General' }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <div class="flex flex-col">
                                    <span class="text-[10px] font-bold text-slate-400 uppercase">Base</span>
                                    <span class="font-bold text-slate-700 uppercase">{{ $item->base_unit }}</span>
                                </div>
                                <div class="w-px h-6 bg-slate-100"></div>
                                <div class="flex flex-col">
                                    <span class="text-[10px] font-bold text-slate-400 uppercase">Conversion</span>
                                    <span class="text-xs text-slate-500 font-medium italic">1 Bag = {{ number_format($item->conversion_rate, 1) }} {{ $item->base_unit }}</span>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex flex-col items-end">
                                <span class="text-xl font-black {{ $item->current_stock <= 50 ? 'text-red-500' : 'text-slate-950' }}">
                                    {{ number_format($item->current_stock, 0) }}
                                </span>
                                <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ $item->base_unit }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4">
