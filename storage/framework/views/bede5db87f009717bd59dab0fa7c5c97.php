...
<?php endif; ?>
        ...
        <template x-teleport="body">
            <?php if (isset($component)) { $__componentOriginal9f64f32e90b9102968f2bc548315018c = $component; } ?>
<?php if (isset($attributes)) { $__attributesOriginal9f64f32e90b9102968f2bc548315018c = $attributes; } ?>
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.modal','data' => ['name' => 'set-farm-weight-modal','title' => 'Set & Adjust Farm Weights','subtitle' => 'Enter farm weight for each entry — total farm weight will be calculated automatically','icon' => 'scale','maxWidth' => '6xl']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
<?php $component->withName('modal'); ?>
<?php if ($component->shouldRender()): ?>
<?php $__env->startComponent($component->resolveView(), $component->data()); ?>
<?php if (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag): ?>
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['name' => 'set-farm-weight-modal','title' => 'Set & Adjust Farm Weights','subtitle' => 'Enter farm weight for each entry — total farm weight will be calculated automatically','icon' => 'scale','maxWidth' => '6xl']); ?>
                <form id="set-farm-weight-form" action="<?php echo e(route('billing.day-load.bulk-update')); ?>" method="POST"
                      x-data="{
                          items: [
                          ...
                          get totalBirdWeight() {
                              return this.items.reduce((sum, item) => sum + item.birdWeight, 0);
                          },
                          get calculatedTotalFarmWeight() {
                              let total = 0;
                              let hasAny = false;
                              this.items.forEach(item => {
                                  if (item.farmWeight !== '' && item.farmWeight !== null && !isNaN(parseFloat(item.farmWeight))) {
                                      total += parseFloat(item.farmWeight);
                                      hasAny = true;
                                  }
                              });
                          ...
                          get totalLoss() {
                              if (!this.calculatedTotalFarmWeight) return null;
                              return (parseFloat(this.calculatedTotalFarmWeight) - this.totalBirdWeight).toFixed(2);
                          }
                      }"
                      class="space-y-6"
                >
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                        ...
                        <div class="rounded-2xl border border-zinc-200/50 dark:border-zinc-700/50 bg-zinc-50 dark:bg-zinc-800/40 p-4 shadow-sm flex flex-col justify-center">
                            <p class="text-[10px] font-bold uppercase tracking-wider text-zinc-400 mb-1">Total Farm Weight (Sum)</p>
                            <p class="font-jetbrains text-2xl font-black text-emerald-600 dark:text-emerald-400" x-text="calculatedTotalFarmWeight ? calculatedTotalFarmWeight + ' kg' : '—'"></p>
                        </div>
                        <div class="rounded-2xl border border-zinc-200/50 dark:border-zinc-700/50 bg-zinc-50 dark:bg-zinc-800/40 p-4 shadow-sm flex flex-col justify-center">
                            <p class="text-[10px] font-bold uppercase tracking-wider text-zinc-400 mb-1">Total Loss</p>
                            <p class="font-jetbrains text-2xl font-black" :class="(calculatedTotalFarmWeight && parseFloat(calculatedTotalFarmWeight) > 0) ? (parseFloat(totalLoss) >= 0 ? 'text-emerald-600' : 'text-rose-600') : 'text-zinc-400'" x-text="totalLoss !== null ? totalLoss + ' kg' : '—'"></p>
                        </div>
                    </div>

                    
                    <div class="overflow-x-auto rounded-2xl border border-zinc-200 dark:border-zinc-800 max-h-[50vh] overflow-y-auto scrollbar-thin scrollbar-thumb-zinc-200 dark:scrollbar-thumb-zinc-800 scrollbar-track-transparent">
                            ...
                            <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800/80 bg-white dark:bg-zinc-900">
                                <?php $__currentLoopData = $allEntries; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $entry): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <tr class="hover:bg-zinc-50/60 dark:hover:bg-zinc-800/40 transition-colors">
                                        ...
                                        <td class="px-4 py-3 text-center font-jetbrains text-xs font-semibold text-zinc-700 dark:text-zinc-300"><?php echo e(number_format((float)$entry->bird_weight, 2)); ?></td>
                                        <td class="px-4 py-3 bg-emerald-50/20 dark:bg-emerald-950/10">
                                            <input
                                                ...
                                                x-model="items.find(i => i.id === <?php echo e($entry->id); ?>).farmWeight"
                                                placeholder="0.00"
                                        ...
                                        <td class="px-4 py-3 text-center font-jetbrains text-xs font-bold"
                                            :class="(items.find(i => i.id === <?php echo e($entry->id); ?>).farmWeight !== '' && !isNaN(parseFloat(items.find(i => i.id === <?php echo e($entry->id); ?>).farmWeight))) ? 'text-rose-600' : 'text-zinc-400'"
                                            x-text="(items.find(i => i.id === <?php echo e($entry->id); ?>).farmWeight !== '' && !isNaN(parseFloat(items.find(i => i.id === <?php echo e($entry->id); ?>).farmWeight))) ? (<?php echo e((float)$entry->bird_weight); ?> - parseFloat(items.find(i => i.id === <?php echo e($entry->id); ?>).farmWeight)).toFixed(2) : '-'">
                                        </td>
                                        <td class="px-4 py-3">
                                            <input
                                                type="text"
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['type' => 'submit','form' => 'set-farm-weight-form','variant' => 'primary','icon' => 'check','class' => 'px-8']); ?>Save & Set All <?php echo $__env->renderComponent(); ?>
<?php endif; ?>
<?php if (isset($__attributesOriginald0f1fd2689e4bb7060122a5b91fe8561)): ?>
<?php $attributes = $__attributesOriginald0f1fd2689e4bb7060122a5b91fe8561; ?>
<?php unset($__attributesOriginald0f1fd2689e4bb7060122a5b91fe8561); ?>
