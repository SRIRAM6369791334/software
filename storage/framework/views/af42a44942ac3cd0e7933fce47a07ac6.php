...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.page-header','data' => ['title' => 'Dealer Billing & Purchases','subtitle' => 'Record daily purchases, calculate weekly cycles, generate invoices, and view ledger logs']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => 'Dealer Billing & Purchases','subtitle' => 'Record daily purchases, calculate weekly cycles, generate invoices, and view ledger logs']); ?>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.data-table','data' => ['headers' => ['Inv No', 'Dealer', 'Period', 'Outstanding & Weekly Payments', 'Total Amount', 'Payment Summary', 'Status', 'Actions']]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['headers' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute(['Inv No', 'Dealer', 'Period', 'Outstanding & Weekly Payments', 'Total Amount', 'Payment Summary', 'Status', 'Actions'])]); ?>
...
<?php endif; ?>
    ...
    <div x-show="activeTab === 'generate_invoice'">
        <?php if (isset($component)) { $__componentOriginal53747ceb358d30c0105769f8471417f6 = $component; } ?>
<?php if (isset($attributes)) { $__attributesOriginal53747ceb358d30c0105769f8471417f6 = $attributes; } ?>
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.card','data' => ['class' => 'max-w-2xl mx-auto','xData' => '{ previewLoaded: false, prevOutstanding: 0, totalPurchases: 0, totalPayments: 0, netInvoice: 0, balanceDue: 0, purchasesCount: 0, discountAmount: 0, billExists: false, overlapExists: false, overlapStart: \'\', overlapEnd: \'\' }','@previewUpdate.window' => '
                previewLoaded = true;
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['class' => 'max-w-2xl mx-auto','x-data' => '{ previewLoaded: false, prevOutstanding: 0, totalPurchases: 0, totalPayments: 0, netInvoice: 0, balanceDue: 0, purchasesCount: 0, discountAmount: 0, billExists: false, overlapExists: false, overlapStart: \'\', overlapEnd: \'\' }','@preview-update.window' => '
                previewLoaded = true;
                prevOutstanding = $event.detail.prevOutstanding;
                totalPurchases = $event.detail.totalPurchases;
                totalPayments = $event.detail.totalPayments;
            ...
            <div class="border-b border-zinc-200 dark:border-zinc-800 pb-4 mb-6">
                ...
                <p class="text-xs text-zinc-500 mt-1">Select a dealer and period to generate a weekly invoice.</p>
            </div>

            <form action="<?php echo e(route('billing.weekly.generate')); ?>" method="POST" id="generate-weekly-bill-form" @submit.prevent="submitWeeklyBill($event)">
                <?php echo csrf_field(); ?>
...
function previewWeeklyBilling(btn) {
    ...
    const performPreview = () => {
        ...
        fetch(`<?php echo e(route('billing.weekly.calculate-preview')); ?>?dealer_id=${dealerId}&period_start=${start}&period_end=${end}&discount_amount=${discount}`)
            .then(res => res.json())
            .then(data => {
                btn.disabled = false;
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (dealerSelect) {
        dealerSelect.addEventListener('change', () => {
            ...
            fetch(`<?php echo e(route('billing.weekly.earliest-unpaid-date')); ?>?dealer_id=${dealerId}`)
                .then(res => res.json())
                .then(data => {
