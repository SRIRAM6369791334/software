...
<?php endif; ?>
    ...
    <?php if(isset($pendingWeightLossBatches) && $pendingWeightLossBatches->isNotEmpty()): ?>
        <div class="mb-8 rounded-2xl border border-amber-200 dark:border-amber-800/60 bg-amber-50/70 dark:bg-amber-900/20 p-5 shadow-sm">
            <div class="flex items-center gap-2 mb-4">
                ...
                <span class="px-2 py-0.5 rounded-full text-xs font-bold bg-amber-100 dark:bg-amber-900/40 text-amber-800 dark:text-amber-300">
                    <?php echo e($pendingWeightLossBatches->count()); ?> Pending
                </span>
            </div>
            <div class="space-y-3">
                <?php $__currentLoopData = $pendingWeightLossBatches; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pBatch): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-white dark:bg-zinc-900 rounded-xl border border-amber-200/70 dark:border-amber-800/40 gap-3 shadow-xs">
                        <div>
                            <p class="font-bold text-sm text-zinc-900 dark:text-zinc-100">
                                Day-Load Batch #<?php echo e($pBatch->id); ?> — <?php echo e($pBatch->billing_date->format('d M Y (l)')); ?>

                            </p>
                            <p class="text-xs text-zinc-500 font-jetbrains mt-1">
                                Loss Weight: <span class="font-bold text-amber-700 dark:text-amber-400"><?php echo e(number_format((float)$pBatch->total_loss_weight, 2)); ?> kg</span>
                                · Financial Amount: <span class="font-bold text-rose-600 dark:text-rose-400">Rs <?php echo e(number_format((float)$pBatch->weight_loss_amount, 2)); ?></span>
                            </p>
                        </div>
                        <div>
                            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('create expenses')): ?>
                            <form method="POST" action="<?php echo e(route('billing.day-load.approve-weight-loss', $pBatch)); ?>" onsubmit="return confirm('Approve Weight Loss Expense of Rs <?php echo e(number_format((float)$pBatch->weight_loss_amount, 2)); ?> for <?php echo e($pBatch->billing_date->format('d M Y')); ?>?')">
                                <?php echo csrf_field(); ?>
                                <?php if (isset($component)) { $__componentOriginald0f1fd2689e4bb7060122a5b91fe8561 = $component; } ?>
<?php if (isset($attributes)) { $__attributesOriginald0f1fd2689e4bb7060122a5b91fe8561 = $attributes; } ?>
...
<?php $component->withAttributes(['type' => 'submit','variant' => 'primary','size' => 'sm','icon' => 'check_circle']); ?>
                                    Approve Weight Loss (Rs <?php echo e(number_format((float)$pBatch->weight_loss_amount, 0)); ?>)
                                 <?php echo $__env->renderComponent(); ?>
<?php endif; ?>
<?php if (isset($__attributesOriginald0f1fd2689e4bb7060122a5b91fe8561)): ?>
<?php $attributes = $__attributesOriginald0f1fd2689e4bb7060122a5b91fe8561; ?>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.badge','data' => ['variant' => $e->category === 'Weight Loss' ? 'rose' : 'zinc']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['variant' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($e->category === 'Weight Loss' ? 'rose' : 'zinc')]); ?><?php echo e($e->category); ?> <?php echo $__env->renderComponent(); ?>
<?php endif; ?>
